local function loadui()
    return function ()
	    ...
	    function menu:CreateMain(Name,Description,KeyBind)
	        local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/ExolutionProject/Other/main/Kavo.lua"))()
	        local Window = Library.CreateLib(string.format("<font color = \"rgb(50,255,255)\">[%s]</font> %s", Name,Description))
	
	        local tabs = {}
	        function tabs:CreateTab(TabName)
	            local Tab = Window:NewTab(TabName)
	            local Section = Tab:NewSection("__________________________________________")
	
	            local controls = {}
	
	            function controls:CreateLabel(LabelText)
	                Section:NewLabel(LabelText)
	            end
	
	            function controls:CreateToggle(ToggleText,callback)
	                Section:NewToggle(ToggleText,"Info is not supported",callback)
	            end
	
	            function controls:CreateButton(ButtonText,callback)
	                Section:NewButton(ButtonText,"Info is not supported",callback)
	            end
	
	            function controls:CreateTextbox(TextboxText,callback)
	                Section:NewTextBox(TextboxText,"Info is not supported",callback)
	            end
	
	            function controls:CreateSlider(SliderText,min,max,callback)
	                Section:NewSlider(SliderText,"Info is not supported",max,min,callback)
	            end
	
	            function controls:CreateBind(BindText,BindDefault,callback)
	                Section:NewKeybind(BindText,"Info is not supported",BindDefault,callback)
	            end
	            return controls
	        end
            ...
            function tabs:CreateNotification(Text,Description,callback)
