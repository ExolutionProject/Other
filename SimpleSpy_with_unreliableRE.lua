--[[
    MODIFICATED BY MORPHIX
    NOW IT HOOKS UNRELIABLE REMOTE EVENT
    THANKS FOR USING
    
    ORIGINAL OWNER - exx1
    ORIGINAL SCRIPT - https://raw.githubusercontent.com/exxtremestuffs/SimpleSpySource/refs/heads/master/SimpleSpy.lua
...
local notoriginalevent = unreliableremoteevent.FireServer
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
    ...
    elseif remote:IsA("UnreliableRemoteEvent") then
        remoteFrame.ColorBar.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
    else
...
function remoteHandler(hookfunction, methodName, remote, args, funcInfo, calling, returnValue)
	local validInstance, validClass = pcall(function()
		return remote:IsA("RemoteEvent") or remote:IsA("RemoteFunction") or remote:IsA("UnreliableRemoteEvent")
	end)
...
function hookRemote(remoteType, remote, ...)
		...
		if validInstance and not (blacklist[remote] or blacklist[remoteName]) then
            ...
            elseif remoteType == "UnreliableRemoteEvent" then
                schedule(
					remoteHandler,
					true,
					remoteType == "UnreliableRemoteEvent" and "fireserver" or "invokeserver",
					remote,
					args,
					funcInfo,
					calling
    ...
    elseif remoteType == "UnreliableRemoteEvent" then
        if remoteHooks[remote] then
			return notoriginalevent(remote, remoteHooks[remote](...))
		end
		return notoriginalevent(remote, ...)
	end
end
...
local newUnreliableFireServer = newcclosure(function(...)
	return hookRemote("UnreliableRemoteEvent", ...)
end, notoriginalevent)
...
function toggleSpy()
	if not toggle then
        ...
        notoriginalevent = hookfunction(unreliableremoteevent.FireServer,newUnreliableFireServer)
	else
		if hookmetamethod then
			if original then
        ...
        hookfunction(unreliableremoteevent.FireServer,notoriginalevent)
	end
end
